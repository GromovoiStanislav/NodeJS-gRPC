...
class MyServiceImplementation {
  DoSomething(
    call: grpc.ServerUnaryCall<example.MyMessage, example.MyMessage>,
    callback: grpc.sendUnaryData<example.MyMessage>
  ): void {
...
server.addService(
  example.UnimplementedMyServiceService.definition,
  // @ts-ignore
