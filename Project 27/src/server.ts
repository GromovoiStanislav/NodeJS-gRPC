...
import {
  IProductServiceServer,
  ProductServiceService,
} from './proto/product_grpc_pb';
import {
  GetProductRequest,
...
const ServerImpl: IProductServiceServer = {
  ...
  getProductViaCategory: (
    call: grpc.ServerWritableStream<GetProductViaCategoryRequest, Product>
  ): void => {
    log(
      `[getProductViaCategory] Request: ${JSON.stringify(
  ...
  getBestProduct: (
    call: grpc.ServerReadableStream<GetProductRequest, Product>,
    callback: grpc.sendUnaryData<Product>
  ): void => {
  ...
  getProducts: (
    call: grpc.ServerDuplexStream<GetProductRequest, Product>
  ): void => {
    call.on('data', (request: GetProductRequest) => {
      const vo = new Product();
