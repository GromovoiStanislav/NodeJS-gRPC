...
export namespace weather {
    ...
    export abstract class UnimplementedWeatherService {
        static definition = {
            ...
            forecast: {
                ...
                requestStream: true,
                responseStream: true,
                requestSerialize: (message: Forecast) => Buffer.from(message.serialize()),
                requestDeserialize: (bytes: Buffer) => Forecast.deserialize(new Uint8Array(bytes)),
                responseSerialize: (message: Forecast.Result) => Buffer.from(message.serialize()),
                responseDeserialize: (bytes: Buffer) => Forecast.Result.deserialize(new Uint8Array(bytes))
            },
            ping: {
                path: "/weather.Weather/ping",
                requestStream: true,
                responseStream: false,
                requestSerialize: (message: Ping) => Buffer.from(message.serialize()),
                requestDeserialize: (bytes: Buffer) => Ping.deserialize(new Uint8Array(bytes)),
                responseSerialize: (message: Ping.Ack) => Buffer.from(message.serialize()),
                responseDeserialize: (bytes: Buffer) => Ping.Ack.deserialize(new Uint8Array(bytes))
            }
        };
        ...
        abstract forecast(call: grpc_1.ServerDuplexStream<Forecast, Forecast.Result>): void;
        abstract ping(call: grpc_1.ServerReadableStream<Ping, Ping.Ack>, callback: grpc_1.sendUnaryData<Ping.Ack>): void;
    }
    export class WeatherClient extends grpc_1.makeGenericClientConstructor(UnimplementedWeatherService.definition, "Weather", {}) {
        ...
        cities: GrpcPromiseServiceInterface<CityQuery, CityQuery.Result> = (message: CityQuery, metadata?: grpc_1.Metadata | grpc_1.CallOptions, options?: grpc_1.CallOptions): Promise<CityQuery.Result> => { if (!metadata) {
            metadata = new grpc_1.Metadata;
        } if (!options) {
            options = {};
        } return new Promise((resolve, reject) => super.cities(message, metadata, options, (error: grpc_1.ServiceError, response: CityQuery.Result) => {
            if (error) {
                reject(error);
            }
            else {
        ...
        get: GrpcStreamServiceInterface<GetTemperature, Temperature> = (message: GetTemperature, metadata?: grpc_1.Metadata | grpc_1.CallOptions, options?: grpc_1.CallOptions): grpc_1.ClientReadableStream<Temperature> => {
            return super.get(message, metadata, options);
        };
        forecast: GrpcChunkServiceInterface<Forecast, Forecast.Result> = (metadata?: grpc_1.Metadata | grpc_1.CallOptions, options?: grpc_1.CallOptions): grpc_1.ClientDuplexStream<Forecast, Forecast.Result> => {
            return super.forecast(metadata, options);
        };
        ping: GrpWritableServiceInterface<Ping, Ping.Ack> = (metadata: grpc_1.Metadata | grpc_1.CallOptions | grpc_1.requestCallback<Ping.Ack>, options?: grpc_1.CallOptions | grpc_1.requestCallback<Ping.Ack>, callback?: grpc_1.requestCallback<Ping.Ack>): grpc_1.ClientWritableStream<Ping> => {
